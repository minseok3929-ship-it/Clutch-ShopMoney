...
public class MoneyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 3 || !"보내기".equals(args[0])) {
            messageUtil.send(player, "§f사용법: /돈 보내기 <닉네임> <금액>");
            return true;
        }
        Player target = Bukkit.getPlayerExact(args[1]);
        Long amount = NumberUtil.parsePositiveLong(args[2]);
        if (target == null || !target.isOnline()) { messageUtil.send(player, "§c대상 플레이어가 오프라인입니다."); return true; }
        ...
        try {
            ...
            messageUtil.send(player, "§f송금 완료: §e" + target.getName() + " " + amount);
            messageUtil.send(target, "§f송금 수신: §e" + player.getName() + " " + amount);
